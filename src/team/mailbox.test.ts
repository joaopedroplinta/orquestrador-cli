...
describe("caixa de mensagens", () => {
  it("entrega mensagens de processos reais, broadcast e deduplica polls", async () => {
    ...
    // Duas chamadas concorrentes compartilham a mesma passagem (flush é
    // reentrante-safe) — a mensagem não pode ser entregue duas vezes.
    await Promise.all([mailbox.flush(), mailbox.flush()]);
  ...
  it("não confia no remetente declarado e ignora destinatário inválido", async () => {
    const root = mkdtempSync(join(tmpdir(), "orquestrador-mail-")); roots.push(root);
